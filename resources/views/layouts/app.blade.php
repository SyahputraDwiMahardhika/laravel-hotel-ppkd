...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .user-name { color: #fff; font-size: .82rem; font-weight: 500; }
        .user-role {
            font-size: .68rem;
            color: var(--gold);
            text-transform: uppercase;
        ...
        .badge-maintenance { background: #E5E7EB; color: #374151; }
        .badge-out_of_order { background: #F3F4F6; color: #6B7280; }
        /* FORMS */
        .form-label {
            font-size: .8rem;
            font-weight: 600;
        ...
        .room-card.available { background: #F0FDF4; border-color: #86EFAC; }
        .room-card.occupied  { background: #FEF2F2; border-color: #FCA5A5; }
        .room-card.cleaning  { background: #FFFBEB; border-color: #FCD34D; }
        .room-card.maintenance { background: #F3F4F6; border-color: #D1D5DB; }
        .room-card:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0,0,0,.08); }
        .room-number { font-family: 'Playfair Display', serif; font-size: 1.5rem; font-weight: 700; }
        /* ALERTS */
...
<body>
    ...
    <p>Jurusan Perhotelan</p>
        ...
        <nav class="sidebar-nav">
            ...
            <a href="{{ route('registrations.create') }}" class="{{ request()->routeIs('registrations.create') ? 'active' : '' }}">
                <i class="fas fa-plus-circle"></i> Registrasi Baru
            </a>
            <a href="{{ route('registrations.index') }}" class="{{ request()->routeIs('registrations.index') ? 'active' : '' }}">
        ...
        <div class="sidebar-footer">
            ...
            <form action="{{ route('logout') }}" method="POST" class="mt-3">
                @csrf
                <button type="submit" class="btn btn-sm w-100" style="background:rgba(255,255,255,.08);color:rgba(255,255,255,.6);border:1px solid rgba(255,255,255,.1);border-radius:8px;font-size:.8rem;">
    ...
    <div class="main-content">
        <header class="topbar">
            ...
            <div class="d-flex align-items-center gap-3">
                ...
                @if(auth()->user()->isAdmin())
                    <span class="badge" style="background:linear-gradient(135deg,var(--gold),#B8903E);color:#fff;font-size:.7rem;padding:5px 10px;border-radius:20px;letter-spacing:.5px;">
                        <i class="fas fa-shield-alt me-1"></i>ADMINISTRATOR
                    </span>
                @else
                    <span class="badge" style="background:#E8F4FD;color:#1A6FA0;font-size:.7rem;padding:5px 10px;border-radius:20px;">
