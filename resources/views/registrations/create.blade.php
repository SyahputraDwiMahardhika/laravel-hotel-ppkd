...
<div class="card mb-4">
    <div class="card-header">
        <h5><span style="background:linear-gradient(135deg,#C9A84C,#B8903E);color:#fff;width:28px;height:28px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;font-size:.8rem;margin-right:10px;">1</span>Informasi Kamar</h5>
    </div>
    <div class="card-body p-4">
        <div class="row g-3">
                ...
                <select class="form-select" name="room_id" id="room_select" required>
                    ...
                    @foreach($rooms as $room)
                        <option value="{{ $room->id }}" data-type="{{ $room->room_type_id }}"
                            ...
                            Kamar {{ $room->room_number }} (Lt. {{ $room->floor }}) — {{ $room->roomType->name }}
                        </option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-2">
                ...
                <input type="number" name="num_guests" class="form-control" min="1" max="10" value="{{ old('num_guests', 1) }}" required>
            </div>
            <div class="col-md-2">
                <label class="form-label">Jumlah Kamar <span class="text-danger">*</span></label>
                <input type="number" name="num_rooms" class="form-control" min="1" max="10" value="{{ old('num_rooms', 1) }}" required>
            </div>

            <!-- Price display -->
            <div class="col-12">
                <div id="price_info" style="background:#FDFAF5;border:1.5px solid #EDE8DE;border-radius:10px;padding:16px;display:none;">
                    <div class="row text-center">
                        <div class="col-md-3">
                            <div style="font-size:.72rem;color:#999;text-transform:uppercase;letter-spacing:1px;">Tipe</div>
                            <div style="font-weight:600;color:#1A1A2E;" id="info_type">—</div>
                        </div>
                        <div class="col-md-3">
                            <div style="font-size:.72rem;color:#999;text-transform:uppercase;letter-spacing:1px;">Harga/Malam</div>
                            <div style="font-weight:600;color:#C9A84C;" id="info_price">—</div>
                        </div>
                        <div class="col-md-3">
                            <div style="font-size:.72rem;color:#999;text-transform:uppercase;letter-spacing:1px;">Jumlah Malam</div>
                            <div style="font-weight:600;color:#1A1A2E;" id="info_nights">—</div>
                        </div>
                        <div class="col-md-3">
                            <div style="font-size:.72rem;color:#999;text-transform:uppercase;letter-spacing:1px;">Total Estimasi</div>
...
<div class="card mb-4">
    <div class="card-header">
        <h5><span style="background:linear-gradient(135deg,#C9A84C,#B8903E);color:#fff;width:28px;height:28px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;font-size:.8rem;margin-right:10px;">2</span>Informasi Menginap</h5>
    </div>
    <div class="card-body p-4">
        <div class="row g-3">
            <div class="col-md-3">
            ...
            <div class="col-md-4">
                ...
                <select name="receptionist_id" class="form-select" required>
                    ...
                    @foreach($receptionists as $r)
                        <option value="{{ $r->id }}" {{ (old('receptionist_id', auth()->id()) == $r->id) ? 'selected' : '' }}>
                            {{ $r->name }} ({{ $r->role }})
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-4">
                ...
                <input type="text" name="deposit_box_number" class="form-control" value="{{ old('deposit_box_number') }}" placeholder="e.g. BOX-021">
            </div>
            <div class="col-md-4">
                <label class="form-label">Issued By</label>
                <input type="text" name="issued_by" class="form-control" value="{{ old('issued_by', auth()->user()->name) }}" placeholder="Nama penerbit">
            </div>
        </div>
    </div>
</div>
...
<div class="card mb-4">
    <div class="card-header">
        <h5><span style="background:linear-gradient(135deg,#C9A84C,#B8903E);color:#fff;width:28px;height:28px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;font-size:.8rem;margin-right:10px;">3</span>Data Tamu</h5>
    </div>
    <div class="card-body p-4">
        <div class="row g-3">
            <div class="col-md-6">
                <label class="form-label">Nama Lengkap <span class="text-danger">*</span></label>
                <input type="text" name="full_name" class="form-control" value="{{ old('full_name') }}" placeholder="Nama sesuai KTP/Passport" required>
            </div>
            <div class="col-md-3">
                <label class="form-label">Pekerjaan</label>
                <input type="text" name="occupation" class="form-control" value="{{ old('occupation') }}" placeholder="e.g. Manager, Dokter">
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-3">
                ...
                <input type="text" name="id_card_number" class="form-control" value="{{ old('id_card_number') }}" placeholder="16 digit NIK" maxlength="16">
            </div>
            <div class="col-md-3">
                <label class="form-label">No. Passport</label>
                <input type="text" name="passport_number" class="form-control" value="{{ old('passport_number') }}" placeholder="A12345678">
            </div>
            <div class="col-md-3">
                <label class="form-label">Nomor Telepon <span class="text-danger">*</span></label>
                <input type="tel" name="phone_number" class="form-control" value="{{ old('phone_number') }}" placeholder="+62 812 3456 7890" required>
            </div>
            <div class="col-md-3">
                <label class="form-label">Nomor Member</label>
                <input type="text" name="member_number" class="form-control" value="{{ old('member_number') }}" placeholder="MBR-XXXX">
            </div>
            <div class="col-md-9">
...
<div class="d-flex gap-3 justify-content-end">
    <a href="{{ route('registrations.index') }}" class="btn btn-outline-secondary" style="border-radius:8px;padding:11px 24px;">
...
function calcPrice() {
    ...
    if (!typeSelect.value || !checkin || !checkout) { priceInfo.style.display = 'none'; return; }

    const selectedOption = typeSelect.options[typeSelect.selectedIndex];
    const price = parseFloat(selectedOption.dataset.price) || 0;
    const nights = Math.max(0, (new Date(checkout) - new Date(checkin)) / 86400000);

    if (nights <= 0) { priceInfo.style.display = 'none'; return; }
